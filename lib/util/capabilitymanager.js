...
//            "platform": "ANY",
    ...
    // Set sauce capabilities
    caps = self.setSauceCaps(caps, config);

    // Process user capabilities
    caps = self.processUserCapabilities(caps, config);

    self.logger.debug('To run the test ' + args.testName + ' with browser:' + caps.browserName + (caps.version ? ',version:' + caps.version : ""));
...
/**
 * Set capabilities for Saucelabs
 * @param {Object} caps
 * @param {Object} config
 */
CapabilityManager.prototype.setSauceCaps = function (caps, config) {
	if (config.isSauceLabs) {
		...
		this.logger.debug('Setting username ' + caps.username);
		this.logger.debug('Setting accessKey ' + caps.accessKey);
	}
	return caps;
};
